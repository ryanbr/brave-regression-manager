...
/// The Settings collapsing panel — extracted so both the Brave Versions
/// tab and the Adblock Lists tab can render it. `id_suffix` keeps the
/// CollapsingHeader / Grid IDs distinct between the two locations.
pub(super) fn render_settings_panel(ui: &mut Ui, state: &mut AppState, id_suffix: &str) {
